    ...
    <div className="relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-96 h-96 opacity-10">
        <div className="w-full h-full bg-gradient-to-tr from-purple-200 to-pink-200 rounded-full transform -translate-x-48 translate-y-48"></div>
            ...
            <div className="max-w-7xl mx-auto pr-4 sm:pr-6 lg:pr-8 py-4 relative z-10">
        {/* Masters Grid */}
        <div className="flex justify-start">
          {mastersData.map((master) => (
                         <div
               key={master.id}
               className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-lg transition-all duration-300 hover:border-purple-200 group max-w-md"
             >
              {/* Master Image */}
              ...
              <div className="mb-4">
                <div className="flex flex-wrap gap-1">
                  {master.services.map((service, index) => (
                    <span
                      key={index}
                      className="text-xs bg-pink-50 text-pink-600 px-2 py-1 rounded-full font-montserrat"
                    >
                      {service}
                    </span>
                  ))}
